...
export const productApi = {
  ...
  getAll(): Promise<Product[]> {
    return axios.get<Product[]>(BASE_URL).then((r) => r.data);
  },

  /** POST /api/products — create a new product */
  create(payload: ProductPayload): Promise<Product> {
    return axios.post<Product>(BASE_URL, payload).then((r) => r.data);
  },

  /** PUT /api/products/{id} — update an existing product */
  update(id: number, payload: ProductPayload): Promise<Product> {
    return axios.put<Product>(`${BASE_URL}/${id}`, payload).then((r) => r.data);
  },

  /** DELETE /api/products/{id} — delete a product */
  remove(id: number): Promise<void> {
    return axios.delete(`${BASE_URL}/${id}`).then(() => undefined);
  },
};
