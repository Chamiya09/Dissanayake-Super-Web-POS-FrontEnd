...
export function generateReorderAccountingPDF(orders, managerName = "Store Manager") {
  ...
  let endY = (doc.lastAutoTable?.finalY ?? 36) + 8;
  if (endY > PH - 34) {
    doc.addPage();
    endY = 26;
  }
  ...
  doc.line(margin, sigY, margin + 68, sigY);
  doc.line(W - margin - 68, sigY, W - margin, sigY);

  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.setTextColor(...THEME.slate500);
  doc.text("Authorized Signature", margin, sigY + 4.5);
  doc.text(managerName, W - margin, sigY + 4.5, { align: "right" });
  doc.setFontSize(7.5);
  doc.text("Accounting Department", W - margin, sigY + 9.5, { align: "right" });

  doc.setFont("helvetica", "normal");
  doc.setFontSize(7.5);
  doc.setTextColor(...THEME.slate500);
  doc.text("Generated by Dissanayake Super Inventory System", W / 2, footerY, { align: "center" });

  const datePart = new Date().toISOString().slice(0, 10).replace(/-/g, "");
  doc.save(`Reorder_Accounting_Report_${datePart}.pdf`);
}

export function generateReorderDetailedBatchPDF(orders, managerName = "Store Manager") {
  const list = Array.isArray(orders) ? orders : [];
  if (!list.length) return;
  ...
  doc.setFontSize(16);
  doc.setTextColor(...THEME.white);
  doc.text("Reorder Detailed Report", margin, 12);
  ...
  list.forEach((order) => {
    const items = buildPdfItems(order, productIndex);
    const orderId = toText(order?.id ?? order?.orderRef, "-");
    const supplier = toText(order?.supplierName, "-");
    const dateLabel = formatOrderDate(order?.orderDate ?? order?.createdAt);
    const status = toText(order?.status, "Pending");
    ...
    items.forEach((item) => {
      const lineTotal = item.lineTotal != null ? item.lineTotal : (item.unitPrice != null ? item.unitPrice * item.quantity : 0);
      ...
      detailRows.push([
        ...
        item.unitPrice == null ? "N/A" : lkrFmt.format(item.unitPrice),
        lineTotal > 0 ? lkrFmt.format(lineTotal) : "N/A",
        status,
      ]);
    });
  });
  ...
  autoTable(doc, {
    startY: 30,
    margin: { left: margin, right: margin },
    head: [["Order ID", "Date", "Supplier", "Product", "SKU", "Qty", "Unit Price (LKR)", "Line Total (LKR)", "Status"]],
    body: detailRows,
    headStyles: {
      fillColor: THEME.cyan700,
      textColor: THEME.white,
  ...
  doc.setFontSize(9);
  doc.setTextColor(...THEME.cyan700);
  doc.text(`Detailed Total (LKR): ${lkrFmt.format(reportTotal)}`, W - 14, endY, { align: "right" });
  ...
  const datePart = new Date().toISOString().slice(0, 10).replace(/-/g, "");
  doc.save(`Reorder_Detailed_Report_${datePart}.pdf`);
}
